...
netsh interface ipv4 set dnsservers name="Wi-Fi" static 1.1.1.1 index=2
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "HideFileExt" -Value 0
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "Hidden" -Value 1
New-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem" -Name "LongPathsEnabled" -Value 1 -PropertyType DWORD -Force

Write-Host "Disabling mouse acceleration..."
Set-ItemProperty -Path "HKCU:\Control Panel\Mouse\MouseSpeed" -Value 0
Set-ItemProperty -Path "HKCU:\Control Panel\Mouse\MouseThreshold1" -Value 0
Set-ItemProperty -Path "HKCU:\Control Panel\Mouse\MouseThreshold2" -Value 0
...
if ($response.body -match "(https[^\s`"']*WPILib_Windows[^\s`"'\)]*)") {
    $url = $matches[1]
    Write-Output $url
